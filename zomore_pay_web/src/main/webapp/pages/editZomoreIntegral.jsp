...
<form action="" method="post">
    ...
    <table align="center">
        ...
        <tr>
            ...
            <td>
                <c:if test="${requestScope.result.integralRuleType==1}">
                    <c:forEach items="${fn:split(result.integralRuleTypeRule, ',')}" var="TypeRule" varStatus="state">
                        <c:if test="${state.count==1}">
                            每消费<input type="text" name="TypeRule" value="${TypeRule}" style="width: 64px">元积1分
                        </c:if>
                        <c:if test="${state.count==2}">
                            <c:if test="${TypeRule=='1'}">
                                <input type="checkbox" name="TypeRule1" value="1" checked> 积分取整数
                            </c:if>
                            <c:if test="${TypeRule!='1'}">
                                <input type="checkbox" name="TypeRule1" value="0"> 积分取整数
                            </c:if>
                        </c:if>
                        <c:if test="${state.count==3}">
                            <c:if test="${TypeRule=='1'}">
                                <input type="checkbox" name="TypeRule2" value="1" checked> 无码商品参与积分
                            </c:if>
                            <c:if test="${TypeRule!='1'}">
                                <input type="checkbox" name="TypeRule2" value="0"> 无码商品参与积分
                            </c:if>
                        </c:if>
                    </c:forEach>
                </c:if>
...
<script type="text/javascript">
    ...
    $(function () {
        $("#submit").click(function () {
            ...
            document.forms[0].action ="${basePath}zomoreIntegral/edit.do";
        });
        //选中指定周期多倍积分
