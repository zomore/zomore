        ...
        <div class="container-fluid">
                    ...
                    <div>
                        <div>
                            <div style="float: left;">
                                <a href="${basePath}zomoreIntegral/addUI.do"
            ...
            <table class="table table-light table-striped shadow-light">
                ...
                <tr style="text-align: center">
                    <th scope="col">操作</th>
                    <th scope="col">积分方式</th>
                    <th scope="col">积分方式的规则</th>
                  <%--  <th scope="col">会员等级多倍积分</th>
                    <th scope="col">会员生日多倍积分</th>
                    <th scope="col">特殊节日多倍积分</th>--%>
                    <th scope="col">指定周期多倍积分</th>
                    <th scope="col">积分兑换方式</th>
                    <th scope="col">是否清空积分</th>
                </tr>
                </thead>
                <tbody>
                <c:forEach items="${requestScope.result}" var="result">
                    <tr style="text-align: center">
                        <th scope="row">
                            <a href="${basePath}zomoreIntegral/editUI.do?id=${result.integralRuleId}"
                               style="color: black">编辑</a>&nbsp;
                            <a href="${basePath}zomoreIntegral/delete.do?ids=${result.integralRuleId}"
                               style="color: black">删除</a>
                        </th>
                        <td>${result.integralRuleType==1?"按金额积分":"其他积分"}</td>
                        <td>
                            <c:forEach items="${fn:split(result.integralRuleTypeRule, ',')}" var="TypeRule" varStatus="state">
                                <c:if test="${state.count==1}">
                                    每消费${TypeRule}元积1分
                                </c:if>
                                <c:if test="${state.count==2}">
                                    <c:if test="${TypeRule=='1'}">
                                        积分取整数
                                    </c:if>
                                </c:if>
                    ...
                    <%--    <td></td>
                        ...
                        <td></td>--%>
                        <td>
                        <c:forEach items="${fn:split(result.integralRuleCycle, ',')}" var="TypeRule" varStatus="state">
                            <c:if test="${state.count==1}">
                                <c:if test="${TypeRule=='1'}">
                                    每周星期
                                </c:if>
                                <c:if test="${TypeRule=='2'}">
                                    每月
                                </c:if>
                                <c:if test="${TypeRule=='3'}">
                                    每月逢
                                </c:if>
                            </c:if>
                            <c:if test="${state.count==2}">
                        ...
                        <td>
                            <c:if test="${result.integralRuleExchange==1}">兑换礼品</c:if>
                            <c:if test="${result.integralRuleExchange==2}">抵扣现金</c:if>
                            <c:if test="${result.integralRuleExchange==3}">无</c:if>
                        </td>
                        <td>${result.integralRuleIsempty==1?'是':'否'}
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
