...
<div class="index-main">
    <table id="table" data-resizable="true">
        ...
        <tr>
            ...
            <th data-field="kc">库存</th>
            <th data-field="dhsl">订货数量</th>
        </tr>
        </thead>
    </table>
</div>
    ...
    <div class="f-bottom f-bottom-bj">
        ...
        <div class="col-sm-4 col-lg-3" style="float: right;">
            <div class="panel p-red">
                <a href="javascript:void(0)" id="getmoney">
            ...
            <div class="modal-body">
                ...
                <div class="table-fex">
                    ...
                    <table id="shopTable" data-resizable="true" class="table  table-bordered ">
                        ...
                        <tbody class="search-list">
                        <c:if test="${requestScope.headshop ne null}">
                            <tr><td>${requestScope.headshop.shopName}</td></tr>
                        </c:if>
                        <c:if test="${requestScope.suppliers ne null}">
                            <c:forEach items="${requestScope.suppliers}" var="supplier">
                                <tr><td>${supplier.zomoreSupplierName}</td></tr>
                            </c:forEach>
                        </c:if>
                        </tbody>
                    </table>
...
<script type="text/javascript">
    ...
    $('#shopLike1').click(function () {
        var name = $('#shopLike').val();
        alert(name);
        // alert(id);
        $('#shopTable').bootstrapTable('destroy');
        $('#shopTable').bootstrapTable({
            url: "${basePath}zomoreCommodityOrdergoods/searchLikeCode.do?name=" + name,
            method: "POST",
            onClickRow: function (row, $element) {
                //响应点击事件
                $('.info').removeClass('info');
    ...
    function huiche() {
        ...
        $.ajax({
            type: "POST",
            url: "${basePath}zomoreCommodity/searchByCode2.do",
            data: {"code": code},
            success: function (msg) {
                ...
                $('#table>tbody>tr>td').each(function () {
                    ...
                    if (index3 == 7 && flag == 1) {
                        ...
                        flag1 = 1;
                        msg[0].dhsl = $(this).text() * 1 + msg[0].dhsl;
                        //$(this).text(msg[0].num);
                    }
                   /* //修改小计
                    if (index3 == 8 && flag == 1) {
    ...
    $('#getmoney').click(function () {
        if ($("#inshop").val() == null) {
            ...
            if (parseFloat(allin).toFixed(2) > 0) {
                ...
                var beforeordergoods = ${sessionScope.zomoreCashierBeforeRight.beforeordergoods}+"";
                if (beforeordergoods=='0'){
                    alert("该收银台没有订货的权限");
                    return false;
                }
               /* hf = $(this).attr("name") + "?money=" + parseFloat(allin).toFixed(2);*/
                ...
                $('#table>tbody>tr>td').each(function () {
                    if (index4 == 5) {
                        conditoutprice = conditoutprice + $(this).text() + ","
                    }
                    index4++;
                    if (index4 == 8) {
                        index4 = 0;
                ...
                conditoutprice = conditoutprice.substring(0, conditoutprice.length - 1);

                var index5 = 0;
                $('#table>tbody>tr>td').each(function () {
                    if (index5 == 3) {
                        commoditytype = commoditytype + $(this).text() + ","
                    }
                    index5++;
                    if (index5 == 8) {
                        index5 = 0;
                ...
                $.ajax({
                    type: "POST",
                    url: "${basePath}zomoreCommodityOrdergoods/insertAll.do",
                    data: {
                        "name": condityname,
                        "code": conditycode,
                        "num": conditynum,
                        "price": condityprice,
                        "outprice":conditoutprice,
                        "type":commoditytype,
                        "zomoreCommodityOrdergoodsOutname": $("#inshop").val()
                    },
                    success: function (msg) {
